...
describe('Given PubsMongoRepo class', () => {
  ...
  describe('When we instantiate it without errors', () => {
    beforeEach(() => {
      ...
      PubsModel.findById = jest.fn().mockReturnValue(mockPub);
    ...
    test('should return a Beer object when given a valid id', async () => {
      ...
      expect(result).toEqual(mockPub);
      expect(PubsModel.findById).toHaveBeenCalledWith(id);
    });
    ...
    test('should add a beer to a pub that already has other beers', async () => {
      const beerId = '12345';
      const pubId = '67890';
      const existingPub = {
        id: pubId,
      ...
      PubsModel.findById = jest.fn().mockImplementationOnce(() => ({
        exec: jest.fn().mockResolvedValueOnce(existingPub),
      }));
      PubsModel.findByIdAndUpdate = jest.fn().mockReturnValueOnce({
        exec: jest.fn().mockResolvedValueOnce(updatedPub),
      });

      const pubsMongoRepo = new PubsMongoRepo();

      const result = await pubsMongoRepo.addBeer(beerId, pubId);
    ...
    test('should not add a beer to the "beers" list when the beer is already in the list', async () => {
      ...
      PubsModel.findById = jest
        .fn()
        .mockReturnValue({ exec: jest.fn().mockResolvedValue(pubs) });
